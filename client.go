	...
	for i := 0; i < l; i++ {
		...
		if inum > 0 {
			encs.New = func() interface{} {
				enc := gotiny.NewEncoderWithTypes(fn.ityps...)
				buf := [32]byte{0, 0, byte(id >> 8), byte(id), 0, 0}
				enc.ResetWithBuf(buf[:6])
				return enc
			}
		}
		if inum == 0 {
...
func GetFnsInfo(sdk interface{}) fnsInfos {
	...
	for i := 0; i < nm; i++ {
		...
		if onum != 0 {
			...
			f.decPool.New = func() interface{} { return gotiny.NewDecoderWithTypes(otpys...) }
		}
...
func (c *Client) getFids(names []string) (fids []int, err error) {
	...
	enc.EncodeByUPtrs(unsafe.Pointer(&names))
	buf := enc.Bytes()
	l := len(buf) - 2 //0,1字节不计算长度
	buf[0] = byte(l >> 8)
	buf[1] = byte(l)
